...
| 最大分歧采样（Max Disagreement） | 选择委员会成员分歧最大的样本 | 基于多个模型之间的不一致性 |
...
#### 3.2.5 最大分歧采样（Max Disagreement Sampling）

**原理**：基于委员会查询（Query-by-Committee）框架，选择多个模型之间分歧最大的样本。
...
**实现细节**：
- 使用3个独立的Logistic Regression模型作为委员会成员
- 每个成员使用不同的随机种子初始化
- 使用modAL库的Committee类实现
- 每个成员都有独立的训练数据和模型参数
...
query_idx, _ = committee.query(X_pool, n_instances=n_to_label)
committee.teach(X=X_pool[query_idx], y=y_pool[query_idx])
```

**设计依据**：
- 使用多个模型组成委员会，增加模型多样性
- 选择委员会分歧最大的样本，这些样本通常包含更多信息
- 基于投票机制选择样本，避免单一模型的偏差

**关键注意事项**：
- Committee的`X_training`属性为`None`，需要通过`learner_list[0].X_training`获取已标注样本数量
- Committee的`teach()`方法会同时更新所有委员会成员
- Committee的`predict()`方法返回投票结果（多数投票）
...
| 标注比例 | 随机采样 | 熵采样 | 边际采样 | 不确定性采样 | 最大分歧采样 |
|----------|----------|--------|----------|--------------|--------------|
| 10% | 0.8551 | **0.8627** | **0.8627** | **0.8627** | 0.8571 |
| 30% | 0.8735 | 0.8725 | 0.8727 | 0.8725 | **0.8767** |
| 50% | 0.8831 | **0.8873** | **0.8873** | **0.8873** | 0.8867 |

**备注**：
- 三种不确定性采样策略（熵采样、边际采样、不确定性采样）在二分类任务中表现一致，因为它们在数学上是单调相关的
- 最大分歧采样（委员会查询）在30%标注比例时表现最佳，展示了委员会查询策略的独特优势
...
| 标注比例 | 准确率 | F1分数 | 精确率 | 召回率 |
|----------|--------|--------|--------|--------|
| 10% | 0.8571 | 0.8569 | 0.8505 | 0.8634 |
| 30% | 0.8767 | 0.8783 | 0.8689 | 0.8880 |
| 50% | 0.8867 | 0.8873 | 0.8794 | 0.8954 |
...
6. **委员会查询策略的独特优势**：最大分歧采样（委员会查询）在30%标注比例时表现最佳（0.8767），超过了所有单一模型的不确定性采样策略。这表明在中等标注资源场景下，委员会查询能够通过多个模型的分歧发现更有价值的样本。

7. **策略选择建议**：
   - 在低标注比例（10%）时，熵采样/边际采样/不确定性采样表现最优
   - 在中等标注比例（30%）时，委员会查询策略表现最优
   - 在高标注比例（50%）时，所有策略的性能趋于接近
...
| 委员会查询 | 基于多个模型分歧选择样本的主动学习框架 |
| 最大分歧采样 | 委员会查询策略之一，选择成员分歧最大的样本 |
