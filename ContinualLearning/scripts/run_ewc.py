...
from src.models import EWCModel
...
def main(config_path):
    ...
    ewc_lambda = float(config.get('model.ewc_lambda', 1000.0))
    ...
    trainer = EWCTrainer(model, config, device)
    ...
    logger.info("=" * 50)
    trainer.compute_fisher_information(train_loader1)
    ...
    logger.info("=" * 50)
    logger.info("Training on Task 2 with EWC")
    logger.info("=" * 50)
    ...
    evaluator.save_results(results, filename='ewc_results.npy')
    
    return results


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run EWC Continual Learning Experiment')
