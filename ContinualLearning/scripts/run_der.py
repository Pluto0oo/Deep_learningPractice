...
from src.models import DERModel
from src.trainer import DERTrainer
...
def main(config_path):
    ...
    buffer_size = config.get('continual_methods.der.buffer_size', 500)
    ...
    trainer = DERTrainer(model, config, device)
    ...
    logger.info("=" * 50)
    logger.info("Storing Task 1 samples in buffer")
    logger.info("=" * 50)
    ...
    model.set_buffer_and_old_model(buffer, old_model)
    
    logger.info("=" * 50)
    logger.info("Training on Task 2 with DER")
    logger.info("=" * 50)
    ...
    evaluator.save_results(results, filename='der_results.npy')
    
    return results


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run DER Continual Learning Experiment')
