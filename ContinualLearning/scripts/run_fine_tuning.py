...
def main(config_path):
    ...
    tokenizer = AutoTokenizer.from_pretrained(config.get('model.name', 'bert-base-uncased'))
    ...
    task1_max_seq_len = config.get('dataset.task1.max_seq_len', 128)
    
    task2_name = config.get('dataset.task2.name', 'ag_news')
    task2_classes = config.get('dataset.task2.num_classes', 4)
    task2_max_seq_len = config.get('dataset.task2.max_seq_len', 128)
    ...
    model = TextClassifier(
        model_name=config.get('model.name', 'bert-base-uncased'),
        num_labels=config.get('model.num_labels', 6),
        dropout_rate=config.get('model.dropout_rate', 0.1)
    )
    
    trainer = FineTuningTrainer(model, config, device)
    ...
    logger.info(f"\n{'=' * 50}")
    logger.info("FINAL RESULTS - Fine Tuning")
    logger.info(f"{'=' * 50}")
    ...
    visualizer.plot_training_curves(
        ...
        ['Task 1', 'Task 2'],
        filename='fine_tuning_curves.png'
    )
    ...
    evaluator.save_results(results, filename='fine_tuning_results.npy')
    
    return results


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run Fine Tuning Continual Learning Experiment')
