...
|------|------------|----------|
| FedAvg | ~78% | 较快 |
| FedProx ($\mu=0.01$) | ~77% | 稍慢 |

**在非IID数据下（Dirichlet $\alpha=0.5$）**：

| 算法 | 最终准确率 | 收敛速度 |
|------|------------|----------|
| FedAvg | ~72% | 较慢 |
| FedProx ($\mu=0.01$) | ~75% | 较快 |

### 6.4 关键发现

1. **FedProx在非IID数据下表现更优**：引入近端惩罚有效缓解客户端漂移问题
2. **$\mu$值选择影响性能**：过大的$\mu$会限制客户端探索，过小则无法有效约束
3. **数据异质性显著影响FedAvg性能**：在严重非IID条件下，FedAvg收敛困难
4. **通信效率是关键瓶颈**：联邦学习的主要开销在于客户端与服务器的通信
...
1. **测试损失曲线**：展示通信轮次与测试损失的关系
2. **测试准确率曲线**：展示通信轮次与测试准确率的关系

### 7.2 曲线解读

- **初始阶段**：模型快速收敛，准确率迅速提升
- **中期阶段**：收敛速度减缓，准确率逐渐趋于稳定
- **后期阶段**：模型达到接近最优状态，波动较小
