...
实验结果保存在以下文件中：
- **完整训练数据**：`results/fedavg_convnet_clients_3_rounds_50_epochs_5/results.json`
- **统计摘要**：`results/fedavg_convnet_clients_3_rounds_50_epochs_5/summary.json`
- **训练曲线**：`results/fedavg_convnet_clients_3_rounds_50_epochs_5/training_curve.png`

### 6.2 FedAvg实验结果（SVHN数据集）

**实验配置**：
- 算法：FedAvg
- 模型：ConvNet（~1.2M参数）
- 数据集：SVHN（73,257训练样本，26,032测试样本）
- 数据划分：IID（独立同分布）
- 客户端数量：3
- 通信轮次：50
- 本地训练：5 epochs
...
| 通信轮次 | 测试准确率 | 测试损失 | 阶段 |
|----------|------------|----------|------|
| 0（初始化） | 10.08% | 2.3035 | 初始状态 |
| 1 | 89.03% | 0.4003 | 快速收敛期 |
| 2 | 91.75% | 0.3685 | 快速收敛期 |
| 3 | 92.14% | 0.3910 | 快速收敛期 |
| 4 | 92.50% | 0.4116 | 快速收敛期 |
| 5 | 92.79% | 0.4417 | 快速收敛期 |
| 10 | 92.84% | 0.5999 | 微调期 |
| 15 | 92.93% | 0.5417 | 微调期 |
| 20 | 92.99% | 0.4995 | 微调期 |
| 25 | 92.80% | 0.4643 | 稳定期 |
...
| 40 | 93.07% | 0.4713 | 稳定期 |
| 45 | 93.04% | 0.4813 | 稳定期 |
| 47 | **93.22%** | 0.4915 | **最佳表现** |
| 50 | 93.07% | 0.4431 | 最终结果 |

**关键统计**：
- **最终准确率**：93.07%（第50轮）
- **最佳准确率**：93.22%（第47轮）
- **平均准确率**：91.17%（50轮平均）
- **准确率标准差**：11.48%（反映收敛过程的波动性）
- **初始提升幅度**：从10.08% → 89.03%（第1轮提升78.95个百分点）
...
- **准确率变化**：92.99% → 93.07%
- **损失变化**：0.4995 → 0.4431
- **特征**：训练趋于稳定，准确率在92.8%-93.2%区间波动
- **波动范围**：仅0.4个百分点（92.82%-93.22%）

### 6.4 算法对比分析

**当前实验**：仅完成FedAvg（IID设置）实验

| 算法 | 数据集 | 数据划分 | 最终准确率 | 最佳准确率 | 状态 |
|------|--------|----------|------------|------------|------|
| FedAvg | SVHN | IID | 93.07% | 93.22% | ✅ 已完成 |
| FedAvg | SVHN | Dirichlet(α=0.5) | - | - | ❌ 待运行 |
| FedProx | SVHN | IID | - | - | ❌ 待运行 |
| FedProx | SVHN | Dirichlet(α=0.5) | - | - | ❌ 待运行 |

### 6.5 关键发现

1. **FedAvg在SVHN IID上表现优异**：
   - 93.07%的准确率表明模型有效学习到数字特征
   - 相比集中式训练（通常95%+），联邦训练性能损失较小

2. **收敛速度极快**：
   - 仅1轮通信就达到89.03%，超过随机初始化的10倍
   - 前5轮即可达到92%以上

3. **训练稳定性好**：
   - 5轮后准确率波动仅0.4%（92.82%-93.22%）
   - 无明显的性能退化或震荡

4. **损失与准确率关系**：
   - 整体呈负相关：损失从2.30降至0.44
   - 部分时段出现"损失上升但准确率仍提高"的现象（如第10轮），这是正常的训练动态

5. **与相关工作对比**：
   - 在IID设置下，FedAvg的93.07%与文献中的结果相当
   - 后续需在非IID设置下评估FedProx的改进效果
...
验证结果文件：
- 训练配置：`summary.json` 包含完整的实验参数
- 每轮结果：`results.json` 包含50轮的测试指标
- 可视化：`training_curve.png` 展示训练曲线
